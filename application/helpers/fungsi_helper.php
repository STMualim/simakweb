...
function sedang_login_ta()
{
    $ci =& get_instance();
    $user_session = $ci->session->userdata('id_ta');
    if($user_session) {
        redirect('dashboard');
    }
}

function tidak_login_ta()
{
    $ci =& get_instance();
    $user_session = $ci->session->userdata('id_ta');
    if(!$user_session) {
...
function cek_admin()
{
    ...
    if($ci->fungsi->user()->level_user == 3) {
        redirect('dashboard');
    }
}
...
function tgl_jam($string){
  $bulanIndo = ['', 'Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Juni', 'Juli', 'Agt', 'Sep' , 'Okt', 'Nov', 'Des'];
