              ...
              <div class="row">
                <div class="col-md-2">
                  <div class="form-group">
                    <input type="text" class="form-control" id="tahunFilter" placeholder="Nama Tahun">
                  </div>
                </div>
                <div class="col-md-2">
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#formData').submit(function(e){
      ...
      var url;
      if (simpan == "tambah") {
        url = "<?= site_url('admin/tahun_ajaran/tambah') ?>"
      } else {
        url = "<?= site_url('admin/tahun_ajaran/edit') ?>"
      }
      $.ajax({
        url: url,
        type: 'post',
    ...
    $('#mdlKonfirm').on('click','#btnKonfirmHapus',function(){
      ...
      $.ajax({
        url: '<?= site_url('admin/tahun_ajaran/hapus') ?>',
        type: 'get',
        data: {id},
        beforeSend: function(){
          loadingBtnOn();
  ...
  function loadData()
  {
    ...
    $('#tblData').DataTable({
      ajax: {
        url: '<?= site_url('admin/tahun_ajaran/load_data') ?>',
        type: 'post',
        data: {tahun}
      },
      columns:
      [
        {data: 'tahun_ta'},
        {data: 'status_ta', render: function(data) {
          return data == 1 ? `<span class="badge bg-teal">Aktif</span>` : `<span class="badge bg-orange">Tidak Aktif</span>`;
        }},
        {data: 'buat_ta', render: function(data) {
          return tglJam(data);
        }},
        {data: "id_ta", orderable: false, searchable: false, render: function(data, type, row) {
          return `<div class="text-center">
                    <button type="button" class="btn btn-rounded btn-teal btn-sm dropdown-toggle" data-toggle="dropdown">
                      <i class="fas fa-cog"></i>
                    </button>
                    <div class="dropdown-menu">
                      <a class="dropdown-item edit-data" href="javascript:void(0)" data-id="`+row.id_ta+`" data-tahun="`+row.tahun_ta+`"><i class="bx bx-edit"></i> Edit</a>
                    </div>
                  </div>`;
        }},
      ],
