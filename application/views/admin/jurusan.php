...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#formData').submit(function(e){
      ...
      var url;
      if (simpan == "tambah") {
        url = "<?= site_url('admin/jurusan/tambah') ?>"
      } else {
        url = "<?= site_url('admin/jurusan/edit') ?>"
      }
      $.ajax({
        url: url,
        type: 'post',
    ...
    $('#mdlKonfirm').on('click','#btnKonfirmHapus',function(){
      ...
      $.ajax({
        url: '<?= site_url('admin/jurusan/hapus') ?>',
        type: 'get',
        data: {id},
        beforeSend: function(){
          loadingBtnOn();
  ...
  function loadData()
  {
    ...
    $('#tblData').DataTable({
      ajax: {
        url: '<?= site_url('admin/jurusan/load_data') ?>',
        type: 'post',
        data: {kode, nama}
      },
      columns:
      [
        {data: 'kode_jurusan'},
        {data: 'nama_jurusan'},
        {data: 'buat_jurusan', render: function(data) {
          return tglJam(data);
        }},
        {data: "id_jurusan", orderable: false, searchable: false, render: function(data, type, row) {
          return `<div class="text-center">
                    <button type="button" class="btn btn-rounded btn-teal btn-sm dropdown-toggle" data-toggle="dropdown">
                      <i class="fas fa-cog"></i>
                    </button>
                    <div class="dropdown-menu">
                      <a class="dropdown-item edit-data" href="javascript:void(0)" data-id="`+row.id_jurusan+`" data-kode="`+row.kode_jurusan+`" data-nama="`+row.nama_jurusan+`"><i class="bx bx-edit"></i> Edit</a>
                      <a class="dropdown-item hapus-data" href="javascript:void(0)" data-id="`+row.id_jurusan+`"><i class="bx bx-trash"></i> Hapus</a>
                    </div>
                  </div>`;
        }},
      ],
